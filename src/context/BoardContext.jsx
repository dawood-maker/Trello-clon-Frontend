...
const initialState = {
  ...
  currentBoard: "board-1",
  cards: {
    "card-1": { id: "card-1", text: "First task for the day", columnId: "column-1" },
    "card-2": { id: "card-2", text: "Currently working on this item", columnId: "column-2" },
    "card-3": { id: "card-3", text: "Item completed and ready for review", columnId: "column-3" },
  },
};
...
export const BoardProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (!loading) localStorage.setItem("trello_clone_data", JSON.stringify(data));
  }, [data, loading]);
  ...
  const addBoard = useCallback((name, color) => {
    ...
    setData(prev => ({
      ...prev,
      boards: { ...prev.boards, [id]: { id, name, color, columnOrder: [], columns: {} } },
      boardOrder: [...prev.boardOrder, id],
      currentBoard: id,
    }));
  }, []);

  const selectBoard = useCallback(boardId => setData(prev => ({ ...prev, currentBoard: boardId })), []);
  const addColumn = useCallback((boardId, title) => {
    const id = uuidv4();
    ...
    setData(prev => ({
      ...
      boards: {
        ...
        [boardId]: {
          ...
          columns: { ...prev.boards[boardId].columns, [id]: newColumn },
        },
      },
    }));
  }, []);
  ...
  const deleteColumn = useCallback((boardId, columnId) => {
    setData(prev => {
      ...
      return {
        ...prev,
        boards: { ...prev.boards, [boardId]: { ...board, columnOrder: board.columnOrder.filter(id => id !== columnId), columns: newColumns } },
        cards: newCards,
      };
    });
  }, []);

  const renameColumn = useCallback((boardId, columnId, title) => {
    setData(prev => ({
      ...prev,
      boards: {
        ...prev.boards,
        [boardId]: { ...prev.boards[boardId], columns: { ...prev.boards[boardId].columns, [columnId]: { ...prev.boards[boardId].columns[columnId], title } } },
      },
    }));
  }, []);
  ...
  const addCard = useCallback((boardId, columnId, text) => {
    const id = uuidv4();
    const newCard = { id, text, columnId };
    setData(prev => ({
      ...prev,
      cards: { ...prev.cards, [id]: newCard },
      boards: { ...prev.boards, [boardId]: { ...prev.boards[boardId], columns: { ...prev.boards[boardId].columns, [columnId]: { ...prev.boards[boardId].columns[columnId], cardIds: [...prev.boards[boardId].columns[columnId].cardIds, id] } } } },
    }));
  }, []);
  ...
  const deleteCard = useCallback(cardId => {
    setData(prev => {
      ...
      return {
        ...
        boards: { ...prev.boards, [boardId]: { ...prev.boards[boardId], columns: { ...prev.boards[boardId].columns, [columnId]: { ...prev.boards[boardId].columns[columnId], cardIds: prev.boards[boardId].columns[columnId].cardIds.filter(id => id !== cardId) } } } },
      };
    });
  }, []);

  const editCardText = useCallback((cardId, text) => setData(prev => ({ ...prev, cards: { ...prev.cards, [cardId]: { ...prev.cards[cardId], text } } })), []);
  const resetAll = useCallback(() => { setData(initialState); localStorage.removeItem("trello_clone_data"); }, []);

  const value = { data, loading, addBoard, selectBoard, addColumn, deleteColumn, renameColumn, addCard, deleteCard, editCardText, resetAll };
